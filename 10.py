...
def canny(image):
    gray = cv.cvtColor(image,cv.COLOR_RGB2GRAY)         ##to convert RGB to gray 
    cv.imshow("gray",gray)
    #blur = cv.GaussianBlur(gray,(5,5),0)                #gaussian blurr to smoothning of image 
    blur = cv.medianBlur(gray,15) 
    cv.imshow("blur",blur)
    canny= cv.Canny(blur,50,150)                        # to detect Edges 
    return canny
...
def display_lines(image,lines):
    ...
    if lines is not None:
        for line in lines:
           # print(line) ## line is two dimentional array 1 raw and 4 column
           x1,y1,x2,y2 = line.reshape(4)
